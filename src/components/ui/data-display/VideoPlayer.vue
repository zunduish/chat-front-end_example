    ...
    <template v-slot="{ player, state }">
      <div class="overlay-container">
        ...
        <div v-if="started" class="controls-container">
          ...
          >
            <!--unmute button-->
            <button v-if="state.muted" class="mute-unmute-buttons" @click="() => handleToggleMute(player)">
            ...
            <!--audio slider-->
            <div class="volume-slider-container" :style="{ opacity: volumeSliderInvisible ? 0 : 1 }">
              <RangeSlider
                @value-changed=" ($event) => handleVolumeSliderChange($event, player)"
                :percentage="volume"
                class="volume-slider"
                aria-label="дууны хэмжээг өөрчлөх гулсагч"
              />
            </div>
          </div>
