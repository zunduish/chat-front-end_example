  ...
  <Modal :open="props.open" :close-modal="props.closeModal">
    <template v-slot:content>
      <div class="w-[25rem] bg-white dark:bg-gray-800 rounded py-6">
        ...
        <!--Caption input-->
        <div class="px-5 py-6">
          <LabeledTextInput placeholder="Гарчиг" type="text" />
        </div>

        <!--Action buttons-->
        <div class="flex w-full px-5">
          <div class="grow flex justify-start">
            <Button class="ghost-primary ghost-text"> Нэмэх </Button>
          </div>

          <Button class="ghost-primary ghost-text mr-4" @click="props.closeModal">
            Гарах
          </Button>
          <Button class="contained-primary contained-text"> Илгээх </Button>
        </div>
      </div>
    </template>
  </Modal>
