<template>
  <div>
    <div class="mb-6 px-5 flex justify-between items-center">
      <!--title-->
      <p class="heading-1 text-color text-[16px]" id="modal-title" tabindex="0">
        <span v-if="conversation.type === 'couple' || props.contact"
        >
          Холбоо барих
<!--          Contact-->
        </span>
        <span v-else-if="conversation.type === 'group'">Бүлэг</span>
        <span v-else-if="conversation.type === 'broadcast'">
        Нэвтрүүлэг
<!--          Broadcast-->
        </span>
...
<!--          title="edit group"-->
    ...
    <div class="w-full py-5 border-t border-gray-100 dark:border-gray-700">
      ...
      <div class="px-5 flex items-center">
        <IconAndText :icon="BellIcon" title="мэдэгдэл" switch />
      </div>
    ...
    <div class="w-full border-t border-gray-100 dark:border-gray-700">
      ...
      >
        <IconAndText :icon="NoSymbolIcon" title="холбоо барихыг хориглох" link />
      </div>
      ...
      <div
        ...
        class="px-5 pt-5 group"
      >
        <IconAndText :icon="TrashIcon" title="харилцагч устгах" link />
      </div>

      <!--(group) exit group-->
      <div v-if="['group', 'broadcast'].includes(conversation.type) && !props.contact" class="px-5 pt-5 flex items-center group">
        <IconAndText :icon="ArrowLeftOnRectangleIcon" title="бүлгээс гарах" link />
      </div>
    </div>
