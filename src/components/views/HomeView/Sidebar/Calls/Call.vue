<template>
  <div>
    <component
      ...
      :aria-label="getCallName(props.call, true) + '-тай дуут дуудлага'"
    ...
    >
      ...
      <div class="w-full flex flex-col">
        <div class="w-full">
          ...
          <div class="flex items-start">
            ...
            <div v-else class="mr-2">
              <PhoneXMarkIcon v-if="props.call.status === 'missed'" class="w-[.875rem] h-[.875rem] text-red-300 dark:text-red-400"/>
              <PhoneArrowUpRightIcon v-else-if="props.call.status === 'sent'" class="w-[.875rem] h-[.875rem] text-green-300 dark:text-green-400"/>
              <PhoneArrowDownLeftIcon v-else-if="props.call.status === 'received'" class="w-[.875rem] h-[.875rem] text-green-300 dark:text-green-400"/>
            </div>
          </div>
        </div>
        ...
        <div>
          <!--recording length-->
          <p v-if="props.active" class="body-2 flex justify-start items-center text-indigo-300">
            {{ props.call.length }}
          </p>
