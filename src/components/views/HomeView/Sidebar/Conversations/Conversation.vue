<template>
  <div class="select-none">
    <button
      :aria-label=" getName(props.conversation) + '-тай хийсэн яриа'"
      tabindex="0"
        ...
        <div class="flex justify-between">
          <div>
            ...
            >
              ...
              />
              <span :class="{ 'text-indigo-400': props.conversation.unread }" v-if="lastMessage">
    ...
    >
      <button
        ...
        aria-label="Харилцааны мэдээллийг харуулах"
        role="menuitem"
      ...
      <button
        class="dropdown-link dropdown-link-primary"
        aria-label="Харилцан яриаг архивт нэмнэ үү"
        role="menuitem"
        @click="handleCloseContextMenu"
      >
        <ArchiveBoxArrowDownIcon class="h-5 w-5 mr-3" />
        Архивын харилцан яриа
      </button>

      <button
        class="dropdown-link dropdown-link-danger"
        aria-label="Харилцан яриаг устгах"
        role="menuitem"
        @click="handleCloseContextMenu"
      >
        <TrashIcon class="h-5 w-5 mr-3" />
        Харилцан яриаг устгах
      </button>
    </Dropdown>
  </div>
</template>
...
const lastMessage = computed(
  () => props.conversation.messages.length > 0 ? props.conversation.messages[props.conversation.messages.length - 1] : undefined,
);
...
const isActive = computed(
  () => store.activeConversation?.id === props.conversation.id,
);
</script>
