<template>
  <SlideTransition animation="shelf-down">
    <div
      ...
      v-if="
        store.activeConversation.pinnedMessage &&
        !store.activeConversation.pinnedMessageHidden"
    >
      <!--pinned message preview-->
      <MessagePreview :message="store.activeConversation?.pinnedMessage" />
...
<!--        aria-label="hide pinned message"-->
        <IconButton
          title="бэхлэгдсэн мессежийг нуух"
          aria-label="бэхлэгдсэн мессежийг нуух"
          @click="handleHidePinnedMessage"
          class="ic-btn-ghost-primary w-7 h-7"
          :class="{
            'mr-3':
              store.user && store.activeConversation?.admins?.includes(store.user.id),
          }"
        >
        ...
        <IconButton
          ...
          class="ic-btn-ghost-primary w-7 h-7"
          title="бэхэлсэн мессежийг хаах"
          aria-label="бэхэлсэн мессежийг хаах"
        >
